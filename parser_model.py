...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden = nn.Linear(in_features = self.embed_size * self.n_features,
    ...
    def embedding_lookup(self, t):
        ...
        x = self.pretrained_embeddings(t) # x is the lookedup embeddings
        x = x.view(x.size()[0],-1)
    ...
    def forward(self, t):
        ...
        x = self.embedding_lookup(t) # 1)
        x = self.embed_to_hidden(x) # 2)
        x = F.relu(x) # 3)
        x = self.dropout(x) # 4)
        logits = self.hidden_to_logits(x) # 5)
